...
import { Dumbbell, PieChart, Calendar, Target, TrendingUp, Clock } from 'lucide-react';
...
export const ExerciseStats: React.FC<ExerciseStatsProps> = ({ className = '' }) => {
  ...
  const loadStats = useCallback(async (period: 'weekly' | 'monthly' | 'yearly') => {
    ...
    if (statsResult.data) {
      // Adaptar os dados do ProgressStats para ExerciseStats
      const exerciseStats: ExerciseStatsData = {
        period: statsResult.data.period,
        exercises_completed: statsResult.data.exercises_completed,
        total_exercise_time: statsResult.data.total_exercise_time,
    ...
    <div className={`card p-6 ${className}`}>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          ...
          <div className="text-center p-4 bg-purple-50 dark:bg-purple-900/20 rounded-lg">
            ...
            <p className="text-2xl font-bold text-purple-600">
              {Object.keys(stats.day_distribution || {}).length}
            </p>
                ...
                <div key={index} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 bg-gray-200 dark:bg-gray-700 rounded-full h-2">
                    <div 
                      ...
                      style={{ 
                        width: `${Math.min((data.exercises_completed / Math.max(...chartData.map(d => d.exercises_completed))) * 100, 100)}%`
                      }}
                    />
                  </div>
      ...
      {!stats && !loading && (
        <div className="text-center py-8">
          ...
          <p className="text-gray-500 dark:text-gray-400">
            Nenhum dado de exercício disponível
          </p>
          <p className="text-sm text-gray-400 dark:text-gray-500">
            Complete exercícios para ver estatísticas
          </p>
        </div>
      )}
      ...
      {error && (
        <div className="text-center py-8">
          <p className="text-red-600 dark:text-red-400 font-medium">
            Erro ao carregar estatísticas de exercícios
          </p>
